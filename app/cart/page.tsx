...
const CartContent = () => {
  ...
  useGSAP(() => {
    ...
    const removeButtonsContainer = document.querySelector('.cart-items-container');
    if (removeButtonsContainer) {
      removeButtonsContainer.addEventListener('mouseenter', (e) => {
      ...
      removeButtonsContainer.addEventListener('mouseleave', (e) => {
        const target = e.target as HTMLElement;
        if (target.classList.contains('remove-btn')) {
          gsap.to(target, {
            scale: 1,
            ...
            { cartItems?.items?.length ||  0 ? cartItems.items.length > 0 ? (
              <ul className="divide-y divide-gray-200">
                {cartItems.items.map((item: any) => (
                  <li key={item.id} className="flex justify-between py-4 cart-item will-change-transform">
                    <div>
          ...
          { cartItems?.items?.length ||  0 ? cartItems.items.length > 0 && (
            <div className="border-t border-gray-200 pt-6 cart-total will-change-transform">
              ...
              <div className="w-full flex justify-end items-center">
                <Link href={"/checkout"} className="bg-yellow-400 text-black py-3 px-4 rounded-lg font-semibold hover:bg-yellow-500 cursor-pointer transition checkout-btn will-change-transform">Checkout</Link>
              </div>
            </div>
          ): ""}
        </main>
